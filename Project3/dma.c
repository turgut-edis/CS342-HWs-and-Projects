...
int dma_init(int m)
{
    ...
    //Each number in bitmap represents 8 byte
...
void *dma_alloc(int size)
{
    ...
    if (size > segSize)
    {
        printf("You cannot allocate larger than the original heap... \n");
        pthread_mutex_unlock(&segment_lock);
...
void dma_free(void *p)
{
    ...
    //Location of the address in bitmap
...
void dma_print_blocks(){
    ...
    printf("\n");
    //Bitmap always starts with 01 pattern because of the representation of bitmap
    for (int i = 0; i < bitmap_size - 1; i+=2){
        
        if(p[i] == 1){ //1x
            if(p[i+1] == 1){ //11
                if(alloc_lock == 1){
                    //beginning of 11 pattern 
                    end_idx = start_idx * word_size;
                    end_addr = &(start_ptr[end_idx]);
                    block_size += 2;
                    block_size *= 8;
                    printf("A, %#018p, 0x%x (%d) \n", end_addr, block_size, block_size);
                    alloc_lock = 0;
                    free_lock = 1;
                    start_idx = i;
                    block_size = 0;
                }
                //continues to 11 pattern 
                block_size += 2;
                free_lock = 1;
                //printf("start_idx: %d\n", start_idx);
            }
...
int dma_give_intfrag(){
    //Return internal fragmentation amount
    return frag_amount;
}
